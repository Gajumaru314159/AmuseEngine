...
TYPED_TEST(RHITest, ComputePipelineState_Readback) {
    ...
    constexpr s32 elementCount = 1024;
    Ref<Buffer> buffer = Buffer::Create(BufferDesc::Structured<int>(
        elementCount,
        BufferState::UnorderedAccess,
        BufferFlag::ShaderResource | BufferFlag::UnorderedAccess | BufferFlag::CopySource));
    ASSERT_NE(buffer, nullptr);
    buffer->update([](void* p) { for (s32 i = 0; i < elementCount;++i) reinterpret_cast<int*>(p)[i] = i; });
    Device::Get()->update();
	table->setResource(0, buffer);

    Ref<Buffer> readback = Buffer::Create(BufferDesc::ByteAddress(
    ...
    cmdListDesc.type = CommandListType::Graphic;
    Ref<CommandList> cmdList = CommandList::Create(cmdListDesc);
    ASSERT_NE(cmdList, nullptr);

    cmdList->begin();
    cmdList->setComputePipelineState(pipeline);
    SetDescriptorTableParam tableParam(table, 0);
    cmdList->setDescriptorTables(&tableParam, 1);
    cmdList->dispatch(DispatchParam{ static_cast<u32>(elementCount), 1, 1 });
    cmdList->addUAVBarrier(buffer);
    cmdList->addBufferBarrier(BufferBarrier{
        buffer,
        BufferState::UnorderedAccess,
        BufferState::CopySource
    });
    cmdList->commitBarriers();
    cmdList->copyBuffer(BufferCopyRegion{
        buffer,
        readback,
        0,
        0,
    ...
    Ref<CommandList> commandLists[] = { cmdList };
    Device::Get()->executeCommandLists(commandLists);
    Device::Get()->waitForIdle();

    const auto* values = static_cast<const int*>(Device::Get()->mapBuffer(readback, CpuAccess::Read));
