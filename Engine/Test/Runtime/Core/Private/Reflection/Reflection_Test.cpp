...
#include <Amuse/Core/Reflection/TypeRegister.h>
...
AMUSE_DEFINE_ENUM_INFO(FruitType) {
...
AMUSE_DEFINE_CLASS_INFO(Nutrients) {
	tag("Description", "栄養素");
...
AMUSE_DEFINE_CLASS_INFO(Food) {
	tag("Description", "食べ物");
}
AMUSE_DEFINE_CLASS_INFO(Fruit) {
...
AMUSE_DEFINE_CLASS_INFO(Vector_int) {
	constructor();
...
AMUSE_DEFINE_CLASS_INFO(MethodTest) {
	constructor();
...
AMUSE_DEFINE_CLASS_INFO(PropertyTest) {
	constructor();
...
AMUSE_DEFINE_CLASS_INFO(FieldTest) {
...
AMUSE_DEFINE_CLASS_INFO(FieldCallbackTest) {
	field("value", &T::value, &T::onEditValue);
}

class PropertyCallbackTest {
public:
	int getValue() const {
		return m_value;
	}
...
AMUSE_DEFINE_CLASS_INFO(PropertyCallbackTest) {
	property("value", &T::getValue, &T::setValue, &T::onEditValue);
}

TEST(TypeBuilder, FieldOnEditCallback) {
	TypeInfoManager manager;
	auto info = manager.find(Type::Get<FieldCallbackTest>());
	ASSERT_NE(info, nullptr);

	auto property = info->findProperty("value");
	ASSERT_NE(property, nullptr);
	ASSERT_TRUE(property->canWrite());

	FieldCallbackTest instance;
	Any owner(instance);
	property->set(owner, 123);

	EXPECT_EQ(instance.value, 123);
	EXPECT_EQ(instance.editCount, 1);
}

TEST(TypeBuilder, PropertyOnEditCallback) {
	TypeInfoManager manager;
	auto info = manager.find(Type::Get<PropertyCallbackTest>());
	ASSERT_NE(info, nullptr);
	...
	PropertyCallbackTest instance;
	Any owner(instance);
	property->set(owner, 456);

	EXPECT_EQ(instance.getValue(), 456);
	EXPECT_EQ(instance.getEditCount(), 1);
}
