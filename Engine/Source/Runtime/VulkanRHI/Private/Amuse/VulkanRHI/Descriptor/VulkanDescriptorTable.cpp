...
namespace Amuse {
	...
	bool VulkanDescriptorTable::setResource(s32 index,const Ref<Buffer>& resource) {
		...
		if (auto p = resource.cast<VulkanBuffer>()) {
			...
			writeDescSet.dstBinding = m_layout->getDesc().items.at(index).index;
			writeDescSet.dstArrayElement = 0;
			writeDescSet.descriptorType =type;
			writeDescSet.descriptorCount = 1;

			if (IsTexelBuffer(type)) {
				if (*p->getTexelView() == VK_NULL_HANDLE) return false;
				writeDescSet.pTexelBufferView = &*p->getTexelView();
			} else {
	...
	bool VulkanDescriptorTable::setResource(s32 index, const Ref<Texture>& resource) {
		...
		if (auto p = resource.cast<VulkanTexture>()) {
			...
			imageInfo.imageLayout = type == vk::DescriptorType::eStorageImage ? vk::ImageLayout::eGeneral : vk::ImageLayout::eShaderReadOnlyOptimal;
	...
	void VulkanDescriptorTable::recordCompute(vk::CommandBuffer commandBuffer, vk::PipelineLayout pipeline, s32 slot) const {
		commandBuffer.bindDescriptorSets(vk::PipelineBindPoint::eCompute, pipeline, slot, *m_set, {});
	}
	...
	bool VulkanDescriptorTable::tryGetRangeType(s32 index, const Ref<Amuse::Buffer>& buffer, vk::DescriptorType& type) const {
		...
		case BindingType::Buffer:
			type = vk::DescriptorType::eUniformTexelBuffer;
			if (auto native = buffer.cast<VulkanBuffer>()) return hasSRV && native->getViewDesc().type == BufferViewType::Buffer && native->getViewDesc().format != TextureFormat::Unknown;
			return false;

		case BindingType::RWBuffer:
			type = vk::DescriptorType::eStorageTexelBuffer;
			if (auto native = buffer.cast<VulkanBuffer>()) return hasUAV && native->getViewDesc().type == BufferViewType::RWBuffer && native->getViewDesc().format != TextureFormat::Unknown;
			return false;
