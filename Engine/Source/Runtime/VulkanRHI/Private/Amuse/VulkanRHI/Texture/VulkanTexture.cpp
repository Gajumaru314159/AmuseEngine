...
#include <Amuse/Core/String/FixedString.h>
...
namespace Amuse {
	...
	static u32 GetViewArrayCount(const TextureDesc& desc, const TextureViewDesc& viewDesc) {
		const s32 total = std::max(desc.arrayNum, 1);
		const s32 count = viewDesc.arrayCount <= 0 ? std::max(total - viewDesc.firstArray, 1) : viewDesc.arrayCount;
		return static_cast<u32>(count);
	}

	static vk::ImageCreateInfo CreateCreateInfo(TextureType type,TextureFormat format,Size size, s32 mipLevel, s32 arrayNum, TextureFlags flags,StringView name) {
		...
		info.usage = vk::ImageUsageFlags(VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT);
		...
		, m_desc(desc)
	{
		...
		// 定義生成
		vk::ImageCreateInfo info = CreateCreateInfo(m_desc.type,m_desc.format, m_desc.size, m_desc.mipLevels, m_desc.arrayNum, m_desc.flags, m_desc.name);
		...
		: m_device(device)
	{
		...
		createView(m_view, m_viewDesc);

		vk::DescriptorImageInfo info({}, *m_view, imageLayout);
		vk::WriteDescriptorSet ddesc;
		ddesc.descriptorType = descriptorType;
		...
		, m_renderDesc(desc)
	{
		...
		if (isDepth) {
			...
			if (TextureFormatUtility::HasStencil(m_renderDesc.format)) {
	...
	bool VulkanTexture::createView(vk::raii::ImageView& view) const {
		return createView(view, m_viewDesc);
	}

	bool VulkanTexture::createView(vk::raii::ImageView& view, const TextureViewDesc& viewDesc) const {
		...
		info.subresourceRange.baseMipLevel = static_cast<u32>(std::max(viewDesc.firstMip, 0));
		info.subresourceRange.levelCount = GetViewMipLevels(viewDesc);
		info.subresourceRange.baseArrayLayer = static_cast<u32>(std::max(viewDesc.firstArray, 0));
		info.subresourceRange.layerCount = GetViewArrayCount(m_desc, viewDesc);

		if (m_desc.type == TextureType::Cube) {
			info.subresourceRange.baseArrayLayer *= 6;
		...
		if (TextureFormatUtility::HasDepth(m_desc.format)) {
			...
			if (TextureFormatUtility::HasStencil(m_desc.format)) {
