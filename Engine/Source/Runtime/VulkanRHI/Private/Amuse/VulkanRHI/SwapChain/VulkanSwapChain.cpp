...
namespace Amuse::RHI {
		...
		: m_device(device)
	{
		...
		if (!desc.window || !desc.window->isValid()) {
	...
	void VulkanSwapChain::update(vk::Queue queue) {
		...
		if (!m_desc.window || !m_desc.window->isValid())return;
...
#include <Amuse/Core/Core.h>
		...
		AMUSE_ASSERT(*m_surface,"サーフェイスが生成されていません");
	...
	void VulkanSwapChain::createResources() {

		{
			...
			m_verices->update(bdesc.size, vertices);
		}
