...
namespace Amuse {
		...
		: m_desc(desc)
	{
		...
		if (!desc.vertexLayout) {
			LOG_FATAL_EX("Graphic", "パイプラインステートの構築に失敗。VertexLayoutが設定されていません。");
			return;
		}
        ...
        for (auto& attribute : vertexLayoutDesc.attributes) {
        ...
        }
		{
			...
			item.binding = 0;
			item.stride = vertexLayoutDesc.vertexStride;
			item.inputRate = vk::VertexInputRate::eVertex;
			AMUSE_ASSERT_EXPR(0 < item.stride);
		}
