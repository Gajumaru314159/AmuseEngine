...
namespace Amuse::RPI {
	...
	void ImGuiData::initializeResource() {
...

		{
			...
			AMUSE_ASSERT_EXPR(m_signature);
		}
		{
			m_table = DescriptorTable::Create({ m_layout });
			AMUSE_ASSERT_EXPR(m_table);
		}
...

		{
			...
			desc.vs = vs;
			desc.ps = ps;
			desc.vertexLayout = VertexLayout::Create(
				"ImGuiLayout",
				sizeof(ImDrawVert),
	...
	void ImGuiData::updateMouse(const Ref<Platform::Window>& window)
	{
		...
		if (bd == nullptr || !window || !window->isValid())
			return;
	...
	void ImGuiData::updateBuffer() {
		...
		}
		{
			...
			m_constantBuffer->update(sizeof(mvp), &mvp, 0);
		}
	...
	void ImGuiPass::AddTask(RenderView& view, ImGuiHandle& handle, ImGuiDelegate func) {
		view.get<ImGuiData>().m_notifier.add(handle, func);
	}
