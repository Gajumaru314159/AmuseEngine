...
`AMUSE_DEFINE_CLASS_INFO`は、クラスの型情報を登録するためのマクロです。  
...
列挙型の情報を登録するためには、`AMUSE_DEFINE_ENUM_INFO`マクロを使用します。  
...
クラス型情報と同様に`AMUSE_REGISTER_RTTI`を使用してマネージャーに型情報を登録してください。

```cpp
AMUSE_REGISTER_RTTI(FruitType);
```
