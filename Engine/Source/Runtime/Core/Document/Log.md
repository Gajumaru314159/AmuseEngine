...
Loggerをインスタンス化してください。なおLoggerはSingletonクラスとして実装されているため、二回目以降のインスタンス化はAssertでクラッシュします。
...
```cpp
AMUSE_ASSERT(entity!=nullptr,"entityがnullptrです。");
```

AssertマクロはLOG_FATAL相当であるため、条件を満たさなかった場合にプログラムが継続できない、もしくは明らかな実装のミスに対してすぐ修正が必要な場合に使用してください。
