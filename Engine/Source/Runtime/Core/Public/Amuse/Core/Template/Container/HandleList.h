...
namespace Amuse {
	...
	public:
		...
		//! @brief ハンドル
		class Handle : Noncopyable, Nonmovable {
		...
		private:
			...
			//! @brief Parentからのremoveを行わずにクリアする
			//! @details Notifierのclear時に呼び出す
			void clear() {
				pParent = nullptr;
				pPrev = nullptr;
				pNext = nullptr;
...
#pragma region Iterator
		...
		public:
			const_iterator(const this_type& x)noexcept :pHandle(const_cast<Handle*>(x.pHandle)) {}                          //!< ムーブコンストラクタ
			const_iterator& operator = (const this_type& x)noexcept { pHandle = x.pHandle; return*this; }					//!< ムーブ代入演算子
			const_reference operator*() const noexcept { return **pHandle; }												//!< インスタンス参照
			const_pointer   operator->() const noexcept { return &(**pHandle); }											//!< メンバアクセス
		...
		protected:
			const_iterator(const Handle* pHandle)noexcept :pHandle(const_cast<Handle*>(pHandle)) {}							//!< ハンドルのポインタから生成(内部用)
		protected:
			Handle* pHandle;																								//!< インスタンス・ポインタ
		};
		...
		public:
			...
			reference operator*() const noexcept { return **(const_cast<Handle*>(pHandle)); }                    //!< インスタンス参照
			pointer   operator->() const noexcept { return &(**(const_cast<Handle*>(pHandle))); }                   //!< メンバアクセス
			this_type& operator++() noexcept { pHandle = pHandle->pNext; return *this; }                                    //!< インクリメント
			this_type& operator--() noexcept { pHandle = pHandle->pPrev; return *this; }                                    //!< デクリメント
			this_type operator++(int)noexcept { auto temp = *this; ++(*this); return temp; }                                //!< 後置インクリメント
			this_type operator--(int)noexcept { auto temp = *this; --(*this); return temp; }                                //!< 後置デクリメント
			...
			bool operator!=(const this_type& rhs)const noexcept { return !(*this == rhs); }                                 //!< 否等価演算子
		protected:
			iterator(const Handle* _pHandle)noexcept :const_iterator(_pHandle) {}                                        //!< ハンドルのポインタから生成(内部用)
...
#pragma endregion
	...
	public:
		...
		bool has(const Handle& handle)const;									// ハンドルがリストに含まれているか判定する
	...
	private:
		...
		void insert_impl(Handle* pPrev, Handle& h);                        // 要素を指定したハンドルの次に挿入
		void insert_front_impl(Handle& h);                                      // 先頭に要素を追加
		void insert_back_impl(Handle& h);                                       // 末尾に要素を追加
		iterator erase_impl(Handle* pHandle)noexcept;                      // 要素の削除
	...
	//! @brief ハンドルがリストに含まれているか判定する 
	template<typename T>
	bool HandleList<T>::has(const Handle& handle)const {
	...
	inline typename HandleList<T>::reference HandleList<T>::emplace_front(Handle& h, Args&&... args) {
		ScopeLock lock(m_lock);
		insert_front_impl(h);
		h.emplace(std::forward<Args>(args)...);
		return *h;
	}
	...
	inline typename HandleList<T>::iterator HandleList<T>::insert(Handle& h, const_iterator position, const T& x) {
		ScopeLock lock(m_lock);
		insert_impl(const_cast<Handle*>(position.pHandle), h);
		h.assign(x);
		return &h;
	}
	...
	template<typename T>
	inline void HandleList<T>::insert_impl(Handle* pPrev, Handle& h) {
		assert(pPrev != nullptr);
		assert(has(*pPrev));// 違うコンテナのイテレータならエラー
	...
	//! @brief 要素の削除
	template<typename T>
	inline typename HandleList<T>::iterator HandleList<T>::erase_impl(Handle* pHandle)noexcept {
