...
namespace Amuse {
	...
	//! @brief 順序あり集合
	template <class T, class TPr = std::less<T>, class TAlloc = Amuse::STLAllocator<T>>
	using Set = std::set<T, TPr, TAlloc>;

	//! @brief 集合
	template <class T, class THasher = std::hash<T>, class TKeyEq = std::equal_to<T>, class TAlloc = Amuse::STLAllocator<T>>
