...
namespace Amuse::Core {
	...
	TLSFBlock* TLSFMapper::allocateFreeBlock(s32 size) {
		...
		if (!slMap) {
			...
			AMUSE_ASSERT(slMap, "内部エラー。割り当て可能なブロックがない状態で処理が継続されています。ロジックを見直してください。");
		...
		AMUSE_ASSERT(pBlock, "内部エラー。allocateFreeBlock()にバグがあります。[pBlock==null]");
		AMUSE_ASSERT(size <= pBlock->capacity, "内部エラー。allocateFreeBlock()にバグがあります。[{}<={}]", size, pBlock->capacity);
	...
	void TLSFMapper::returnSurplusBlock(TLSFBlock& block, s32 size) {
		...
		AMUSE_ASSERT(size, "サイズ0で分割できません。");
		AMUSE_ASSERT(!block.allocated, "アロケート済みのブロックは分割できません。");
		AMUSE_ASSERT(!block.pFreePrev, "フリーブロックに接続されています。");
		AMUSE_ASSERT(!block.pFreeNext, "フリーブロックに接続されています。");
		...
		AMUSE_ASSERT(!m_freeList.empty(), "TLSFMapperの容量が不足しています。");
	...
	void TLSFMapper::separateFreeList(TLSFBlock& block) {
		...
		AMUSE_ASSERT(!block.allocated, "フリーリストにないブロックは分離できません。");
	...
	TLSFBlock* TLSFMapper::mergeFreeBlocks(TLSFBlock& block1, TLSFBlock& block2) {
		...
		AMUSE_ASSERT(!block1.allocated, "アロケート済みのブロックはマージできません。");
		AMUSE_ASSERT(!block2.allocated, "アロケート済みのブロックはマージできません。");
		AMUSE_ASSERT(block1.pNext == &block2 && &block1 == block2.pPrev, "連続していないブロックはマージできません。");
	...
	void TLSFMapper::addFreeBlock(TLSFBlock& block) {
		...
		AMUSE_ASSERT(!block.allocated, "アロケート済みのブロックはフリーリストに追加できません。");
		AMUSE_ASSERT(!block.pFreePrev && !block.pFreeNext, "連結されているブロックはフリーリストに追加できません。");
