...
#include <Amuse/DirectX12RHI/GraphicFile/DirectX12GraphicFile.h>
#include <Amuse/Platform/System.h>

#ifdef AMUSE_DEBUG
#include <pix3.h>
#include <Amuse/DirectX12RHI/Utility/PIXModule.h>
#endif
...
namespace Amuse::RHI {
		...
		, m_dx12config(dx12config ? *dx12config : DirectX12RHIConfig{})
	{
		...
		AMUSE_DEBUG_CONTEXT(m_commandQueue->setName("SystemCommandQueue"));
	...
	DirectX12Device::~DirectX12Device() {
		...
		AMUSE_DEBUG_CONTEXT(finalizeDebugMessageCallback());
	...
	//! @brief  フェンスを生成
	Ref<Fence> DirectX12Device::createFence(const FenceDesc& desc) {
		SAFE_CREATE(Fence, DirectX12Fence, *this, desc);
	}

	//! @brief  コマンドリストを実行
	u64 DirectX12Device::executeCommandLists(Span<Ref<CommandList>> commandLists, CommandQueueType) {
		...
		for (const auto& commandList : commandLists) {
			if (auto native = commandList.cast<DirectX12CommandList>()) {
				for (const auto& query : native->getEventQueries()) {
					if (auto event = query.cast<DirectX12EventQuery>()) {
	...
	void DirectX12Device::waitForIdle(CommandQueueType) {
		m_commandQueue->wait();
	}

	//! @brief  キュー間同期を設定
	void DirectX12Device::queueWait(CommandQueueType, CommandQueueType, u64 fenceValue) {
		m_commandQueue->queueWait(fenceValue);
	}
	...
	Ref<Sampler> DirectX12Device::createSampler(const SamplerDesc& desc) {
		return m_samplerCache.getOrCreate(*this,desc);
	}
	...
	//! @brief GPUイベントクエリを生成
	Ref<EventQuery> DirectX12Device::createEventQuery() {
		SAFE_CREATE(EventQuery, DirectX12EventQuery, *this);
	}

	//! @brief GPUイベントクエリが完了済みか
	bool DirectX12Device::pollEventQuery(const Ref<EventQuery>& query) {
		if (auto event = query.cast<DirectX12EventQuery>()) {
			const auto fenceValue = event->getFenceValue();
			return fenceValue == 0 || m_commandQueue->getCompletedValue() >= fenceValue;
		}
		return false;
	}

	//! @brief GPUイベントクエリを待機
	void DirectX12Device::waitEventQuery(const Ref<EventQuery>& query) {
		if (auto event = query.cast<DirectX12EventQuery>()) {
			const auto fenceValue = event->getFenceValue();
			if (fenceValue != 0) {
	...
	//! @brief GPUタイマークエリを生成
	Ref<TimerQuery> DirectX12Device::createTimerQuery() {
		SAFE_CREATE(TimerQuery, DirectX12TimerQuery, *this);
	}

	//! @brief GPUタイマークエリが取得可能か
	bool DirectX12Device::pollTimerQuery(const Ref<TimerQuery>& query) {
		return query.cast<DirectX12TimerQuery>() != nullptr;
	}

	//! @brief GPUタイマークエリの計測時間を取得
	f32 DirectX12Device::getTimerQueryTime(const Ref<TimerQuery>& query) {
	...
	bool DirectX12Device::supports(TextureFormat format, TextureType type)const {
		...
		if (type == TextureType::Cube && !(result.Support1 & D3D12_FORMAT_SUPPORT1_TEXTURE2D)) return false;
	...
	RHIFeatures DirectX12Device::getFeatures() const {
		...
		features.stagingTexture = false;

		{
			D3D12_FEATURE_DATA_D3D12_OPTIONS options{};
			if (SUCCEEDED(m_device->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS, &options, sizeof(options)))) {
				features.conservativeRasterization = options.ConservativeRasterizationTier != D3D12_CONSERVATIVE_RASTERIZATION_TIER_NOT_SUPPORTED;
			}
		}
		{
			D3D12_FEATURE_DATA_D3D12_OPTIONS6 options{};
			if (SUCCEEDED(m_device->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS6, &options, sizeof(options)))) {
				features.variableRateShading = options.VariableShadingRateTier != D3D12_VARIABLE_SHADING_RATE_TIER_NOT_SUPPORTED;
			}
		}
		{
			D3D12_FEATURE_DATA_D3D12_OPTIONS5 options{};
			if (SUCCEEDED(m_device->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS5, &options, sizeof(options)))) {
				features.rayTracing = options.RaytracingTier != D3D12_RAYTRACING_TIER_NOT_SUPPORTED;
			}
		}
		{
			D3D12_FEATURE_DATA_D3D12_OPTIONS7 options{};
			if (SUCCEEDED(m_device->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS7, &options, sizeof(options)))) {
				features.meshShader = options.MeshShaderTier != D3D12_MESH_SHADER_TIER_NOT_SUPPORTED;
			}
		}
		return features;
	}
	...
	bool DirectX12Device::supports(TextureFormat format, FormatSupport usage) const {
		if (format == TextureFormat::Unknown) return false;
		D3D12_FEATURE_DATA_FORMAT_SUPPORT result{};
		result.Format = TypeConverter::Convert(format);
		if (FAILED(m_device->CheckFeatureSupport(D3D12_FEATURE_FORMAT_SUPPORT, &result, sizeof(result)))) return false;
		...
		case FormatSupport::Texture:
			return result.Support1 & D3D12_FORMAT_SUPPORT1_TEXTURE2D;
		case FormatSupport::RenderTarget:
			return result.Support1 & D3D12_FORMAT_SUPPORT1_RENDER_TARGET;
		case FormatSupport::DepthStencil:
			return result.Support1 & D3D12_FORMAT_SUPPORT1_DEPTH_STENCIL;
		case FormatSupport::ShaderResource:
			return result.Support1 & D3D12_FORMAT_SUPPORT1_SHADER_SAMPLE;
		case FormatSupport::UnorderedAccess:
			return result.Support1 & D3D12_FORMAT_SUPPORT1_TYPED_UNORDERED_ACCESS_VIEW;
		case FormatSupport::Copy:
			return true;
		case FormatSupport::Resolve:
			return result.Support1 & D3D12_FORMAT_SUPPORT1_MULTISAMPLE_RESOLVE;
		}
		return false;
	}
	...
	void DirectX12Device::allocateHandle(DescriptorHeapType type, DescriptorHandle& handle, s32 size) {
		if (!m_descriptorHeaps.contains(type)) {
			AMUSE_ABORT("不正なDescriptorHeapType");
		}
	...
	bool DirectX12Device::initializeDXGIDevice() {
		...
		AMUSE_DEBUG_CONTEXT(m_device->SetName(L"System Device"));
		...
		if (SUCCEEDED(m_device->QueryInterface(IID_PPV_ARGS(infoQueue.ReleaseAndGetAddressOf())))) {
			...
			// D3D12 エラー発生時にブレーク
			if (m_dx12config.breakWithWarning)infoQueue->SetBreakOnSeverity(D3D12_MESSAGE_SEVERITY_WARNING, FALSE);
			infoQueue->SetBreakOnSeverity(D3D12_MESSAGE_SEVERITY_ERROR, FALSE);
		}

		AMUSE_DEBUG_CONTEXT(if (!initializeDebugMessageCallback()) return false);
