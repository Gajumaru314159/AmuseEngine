...
namespace Amuse {
		...
		: m_desc(desc)
	{
		...
		if (!desc.vertexLayout) {
			LOG_ERROR_EX("Graphic", "パイプラインステートの構築に失敗。VertexLayoutが設定されていません。");
			return;
		}
		...
		// シェーダ
		{
			...
			if (m_desc.ps) {
				auto& shader = *m_desc.ps.cast<DirectX12Shader>();
				gpsd.PS = CD3DX12_SHADER_BYTECODE(shader.getBinaryData(), shader.getBinarySize());
			}
		}
		...
		// 頂点レイアウト
		auto& vertexLayoutDesc = desc.vertexLayout->getDesc();
		Vector<D3D12_INPUT_ELEMENT_DESC> attributes;
		gpsd.InputLayout.NumElements = gsl::narrow_cast<UINT>(vertexLayoutDesc.attributes.size());
		attributes.reserve(gpsd.InputLayout.NumElements);
		for (auto& attr : vertexLayoutDesc.attributes) {
			attributes.push_back({});
			SetupVertexLayout(attributes.back(), attr);
		}
		gpsd.InputLayout.pInputElementDescs = attributes.data();
		AMUSE_ASSERT(0 < vertexLayoutDesc.vertexStride,"頂点のサイズは0より大きい必要があります");
	...
	//! @brief      名前を取得
	const String& DirectX12PipelineState::getName()const {
		return m_desc.name;
	}
