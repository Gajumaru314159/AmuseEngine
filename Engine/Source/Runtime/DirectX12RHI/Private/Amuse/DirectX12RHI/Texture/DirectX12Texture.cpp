...
namespace Amuse {
	...
	static s32 GetViewArrayCount(const TextureDesc& desc, const TextureViewDesc& viewDesc) {
		const s32 total = std::max(desc.arrayNum, 1);
		if (viewDesc.arrayCount <= 0) return std::max(total - viewDesc.firstArray, 1);
		return viewDesc.arrayCount;
	}

	static u32 GetRangeArrayCount(const TextureDesc& desc, const TextureSubresourceRange& range) {
		const u32 total = static_cast<u32>(std::max(desc.arrayNum, 1));
		const u32 first = std::min(range.baseArrayLayer, total);
		if (range.arrayLayerCount == TextureSubresourceRange::All) {
			return std::max(1u, total - first);
		}
		return std::max(1u, std::min(range.arrayLayerCount, total - first));
	}
	...
	static D3D12_RESOURCE_DESC CreateResourceDesc(TextureType type,TextureFormat format,Size size,s32 mipLevel,s32 arrayNum,StringView name) {
		...
		auto nativeFormat = TypeConverter::Convert(format, TextureFormatUtility::HasDepth(format));
		...
		: m_device(device)
	{
		...
		case TextureViewType::Texture:
			if (flags & TextureFlag::ShaderResource) {
				createSRV(m_handle.getCpuHandle(), m_viewDesc);
			} else {
		...
		case TextureViewType::RWTexture:
			if (flags & TextureFlag::UnorderedAccess) {
				createUAV(m_handle.getCpuHandle(), m_viewDesc);
			} else {
		...
		, m_renderDesc(desc)
	{
		...
		if (isColor) {
			AMUSE_ASSERT(!TextureFormatUtility::HasDepth(m_renderDesc.format), "カラーに非対応なフォーマットです。");
			resourceDesc.Flags |= D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET;
			m_state = D3D12_RESOURCE_STATE_RENDER_TARGET;
			clearValue = &colorClearValue;
		}
		if (isDepth) {
			AMUSE_ASSERT(TextureFormatUtility::HasDepth(m_renderDesc.format), "デプス・ステンシルに非対応なフォーマットです。");
	...
	void DirectX12Texture::initialize() {
		...
		m_viewDesc.firstMip = 0;
		m_viewDesc.mipLevels = 0;
		m_viewDesc.firstArray = 0;
		m_viewDesc.arrayCount = 0;
		m_viewDesc.planeSlice = 0;
	...
	void DirectX12Texture::createSRV(D3D12_CPU_DESCRIPTOR_HANDLE handle)const {
		createSRV(handle, m_viewDesc);
	}


	//! @brief      SRVを生成
	void DirectX12Texture::createSRV(D3D12_CPU_DESCRIPTOR_HANDLE handle, const TextureViewDesc& viewDesc)const {
		...
		auto mipLevels = GetViewMipLevels(viewDesc);
		const auto firstMip = std::max(viewDesc.firstMip, 0);
		const auto firstArray = std::max(viewDesc.firstArray, 0);
		const auto arrayCount = GetViewArrayCount(m_desc, viewDesc);
		...
		case TextureType::Texture2D:
			if (0 < m_desc.arrayNum) {
				...
				desc.Texture2DArray.PlaneSlice = viewDesc.planeSlice;
			} else {
				desc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D;
				desc.Texture2D.MostDetailedMip = firstMip;
				desc.Texture2D.MipLevels = mipLevels;
				desc.Texture2D.PlaneSlice = viewDesc.planeSlice;
			}
			break;
		case TextureType::Texture3D:
			if (0 < m_desc.arrayNum) {
				AMUSE_ABORT("Texture3Dは配列にできません。IsInvalid()の条件を修正してください。");
			} else {
		...
		case TextureType::Cube:
			if (0 < m_desc.arrayNum) {
				...
				desc.TextureCubeArray.First2DArrayFace = firstArray * 6;
				desc.TextureCubeArray.NumCubes = arrayCount;
			} else {
	...
	//! @brief      UAVを生成
	void DirectX12Texture::createUAV(D3D12_CPU_DESCRIPTOR_HANDLE handle, s32 mipSlice)const {
		auto viewDesc = m_viewDesc;
		viewDesc.firstMip = mipSlice;
		createUAV(handle, viewDesc);
	}


	//! @brief      UAVを生成
	void DirectX12Texture::createUAV(D3D12_CPU_DESCRIPTOR_HANDLE handle, const TextureViewDesc& viewDesc)const {
		...
		const auto mipSlice = std::max(viewDesc.firstMip, 0);
		const auto firstArray = std::max(viewDesc.firstArray, 0);
		const auto arrayCount = GetViewArrayCount(m_desc, viewDesc);
		...
		case TextureType::Texture3D:
			if (1 < m_desc.arrayNum) {
				AMUSE_ABORT("Texture3Dは配列にできません。");
			}
			else {
			...
			break;
		case TextureType::Cube:
			AMUSE_ABORT("TextureCubeはUAVに使用できません");
			break;
		default:
			AMUSE_ABORT("不明なテクスチャタイプ");
			break;
	...
	//! @brief RTVを生成
	void DirectX12Texture::createRTV(D3D12_CPU_DESCRIPTOR_HANDLE handle, const TextureSubresourceRange& range)const {
		D3D12_RENDER_TARGET_VIEW_DESC viewDesc{};
		viewDesc.Format = m_resource->GetDesc().Format;
		const u32 mipSlice = range.baseMipLevel;
		const u32 firstArray = range.baseArrayLayer;
		const u32 arrayCount = GetRangeArrayCount(m_desc, range);
		...
		case TextureType::Texture2D:
		case TextureType::Cube:
			if (0 < m_desc.arrayNum || m_desc.type == TextureType::Cube) {
				viewDesc.ViewDimension = D3D12_RTV_DIMENSION_TEXTURE2DARRAY;
				viewDesc.Texture2DArray.MipSlice = mipSlice;
				viewDesc.Texture2DArray.FirstArraySlice = m_desc.type == TextureType::Cube ? firstArray * 6 : firstArray;
				viewDesc.Texture2DArray.ArraySize = m_desc.type == TextureType::Cube ? arrayCount * 6 : arrayCount;
		...
		case TextureType::Texture3D:
			...
			viewDesc.Texture3D.WSize = std::max(m_desc.size.depth, 1);
			break;
		}

		m_device.getNative()->CreateRenderTargetView(m_resource.Get(), &viewDesc, handle);
	}

	//! @brief DSVを生成
	void DirectX12Texture::createDSV(D3D12_CPU_DESCRIPTOR_HANDLE handle, const TextureSubresourceRange& range)const {
		D3D12_DEPTH_STENCIL_VIEW_DESC viewDesc{};
		viewDesc.Format = TypeConverter::Convert(m_desc.format);
		viewDesc.Flags = D3D12_DSV_FLAG_NONE;
		const u32 mipSlice = range.baseMipLevel;
		const u32 firstArray = range.baseArrayLayer;
		const u32 arrayCount = GetRangeArrayCount(m_desc, range);
		...
		m_device.getNative()->CreateDepthStencilView(m_resource.Get(), &viewDesc, handle);
	}
