...
namespace Amuse::RHI
{
	...
	//! @brief  アップロード要素を追加
	void DirectX12BufferUploader::add(BlobView blob, const ComPtr<ID3D12Resource>& dest, size_t offset, D3D12_RESOURCE_STATES afterState) {
		...
		request.afterState = afterState;

		block.blob.append(blob.data(), blob.size());

		m_enteredResources[dest.Get()] = afterState;

	}

	void DirectX12BufferUploader::add(const Buffer::CopyFunc& func, size_t size, const ComPtr<ID3D12Resource>& dest, size_t offset, D3D12_RESOURCE_STATES afterState) {
	...
	void DirectX12BufferUploader::update(ID3D12GraphicsCommandList& commandList) {
		...
		m_barriers.clear();
		for (auto& [resource, afterState] : m_enteredResources) {
